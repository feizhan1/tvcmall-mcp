...
const FAKE_PRODUCT_DETAILS: ProductDetail[] = [
  {
...
const FAKE_PRODUCTS: ProductSummary[] = FAKE_PRODUCT_DETAILS.map(({ moq, weight_kg, dimensions_cm, attributes, images, ...summary }) => summary);
...
export class FakeProductClient implements ProductClient {
  ...
  async getProductDetail(productId: string, _session: StoredAuthSession): Promise<ProductDetail | null> {
    return FAKE_PRODUCT_DETAILS.find((product) => product.id === productId || product.sku === productId) ?? null;
  }
}
