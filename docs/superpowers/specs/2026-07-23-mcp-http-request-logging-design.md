...
- `info`（默认）记录服务已监听，以及每个 MCP 请求完成后的 method、HTTP status、JSON-RPC method、请求类别和耗时。
...
只允许输出事件名、HTTP method/status、JSON-RPC method、请求类别、稳定错误码、耗时和配置的监听 host/port/path。

## Architecture

新增独立的 logger 模块，负责日志等级过滤、JSON 序列化和 stderr 输出。`src/index.ts` 根据 `loadRuntimeConfig()` 创建 logger，并将其传给 HTTP server。HTTP server 在请求结束后产生一条汇总日志；它从已解析的 JSON-RPC body 中只提取 `method`，不读取 `params`，并在安全错误响应中记录稳定错误码。
...
1. 默认 `info` 会输出启动和成功的 MCP 请求汇总日志。
2. `silent` 不输出日志。
