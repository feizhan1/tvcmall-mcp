...
当前 v0.1 提供商品、订单、物流、运费和积分的只读查询。MCP Server 将同一 PAT 原样传给现有 TVCMall WebApi；WebApi、ApplicationServices 与 RDS 负责 PAT 校验、scope 和 method + normalized route allowlist 授权。
...
在 MCP Client 中配置远程 URL，并让客户端的 secret 管理能力展开 PAT：
...
`Authorization` 必须出现在每个 `POST`、`GET` 和 `DELETE /mcp` 请求中。初始化成功后，客户端还必须在后续请求携带服务端返回的 `Mcp-Session-Id` 和同一 PAT。不要把 PAT 写进仓库、普通配置模板、日志或对话。
...
生产环境应把服务部署在 TLS 终止层后，只暴露 `/mcp` 和无敏感信息的 `/healthz`。反向代理与应用日志都不得记录 `Authorization`。MCP Server 不配置服务器共享 PAT；PAT 只能由各 MCP Client 在请求头中提供。
...
```bash
curl https://mcp.tvcmall.com/mcp \
  -H 'Authorization: Bearer tmcp_v1_{tokenId}.{secret}' \
...
成功响应包含 `Mcp-Session-Id`。后续请求必须同时携带该 session ID 与同一 `Authorization`；替换 PAT 会返回 `401 AUTH_REQUIRED`。
...
| `429 RATE_LIMITED` | WebApi 正在限流；按安全的 `Retry-After` 或稍后重试 |
...
- MCP HTTP 层只校验 Bearer/PAT 基本格式、请求 schema、session 指纹和容量/idle TTL，不判断用户、scope 或过期时间。
