...
> 当前状态：v0.1 最小 TypeScript 骨架已初始化，已包含 stdio MCP server、商品域假数据 tools（`tvcmall_search_products`、`tvcmall_get_product_detail`、`tvcmall_estimate_shipping`）、基础 CLI、系统凭证库 token store、fake auth 的 login/refresh/logout/me、过期自动 refresh、测试与构建脚本。真实业务 API 仍待接入。
...
4. 确认订单、物流接口是否能用 Bearer token 调用。
